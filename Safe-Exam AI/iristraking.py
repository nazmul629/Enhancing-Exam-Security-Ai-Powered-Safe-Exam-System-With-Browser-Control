...
with mp_face_mesh.FaceMesh(
    ...
    while True:
        ...
        if results.multi_face_landmarks:
            ...
            iris_up_down_pos , u_d_ratio = iris_up_down_position(center_left,mesh_points[L_TOP] ,mesh_points[L_DOWN][0])

            print(u_d_ratio)
            # print(iris_pos)

            if  iris_pos== "Center":
                cv2.putText(frame, f"Iris Positon: {iris_pos} {ratio:.2f}",(30,30),cv2.FONT_HERSHEY_PLAIN,1.2,(0,255,0),1,cv2.LINE_AA)
            else:
                cv2.putText(frame, f"Iris Positon: {iris_pos} {ratio:.2f}",(30,30),cv2.FONT_HERSHEY_PLAIN,1.2,(0,0,255),1,cv2.LINE_AA)
